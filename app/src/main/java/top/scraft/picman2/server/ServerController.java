...
public class ServerController {
  ...
  private <T> Result<T> request(Request.Builder req, Type type) {
    try (Response response = httpClient
      ...
      if (response.body() != null) {
        // FIXME: json格式化失败的情况
        Result<T> r = gson.fromJson(response.body().charStream(), type);
        if (r != null) {
          return r;
        }
      }
      return new Result<>(response.code(), response.code() + "", null);
    } catch (IOException e) {
      e.printStackTrace();
  ...
  @NonNull
  @EverythingIsNonNull
  public Result<PictureDetail> getPictureMeta(@NonNull String pid) {
    String api = String.format(Locale.US, "/api/pic/%s", pid);
    return request(requestBuilder(api).get(), new TypeToken<Result<PictureDetail>>() {
    }.getType());
  }
  ...
  @NonNull
  @EverythingIsNonNull
  public Result<PictureDetail> updatePictureMeta(long lid, String pid, String description, Set<String> tags) {
    ...
    return request(requestBuilder(path).put(body),
  ...
  public Result<Object> uploadPicture(long lid, String pid, File picture) {
    ...
    RequestBody requestBody = new MultipartBody.Builder()
        ...
        .build();
    return request(requestBuilder(path).post(requestBody),
        new TypeToken<Result<Object>>() {
        }.getType());
  }
